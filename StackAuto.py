...
class Transicao():
  ...
  def executa(self, entrada, pilha):
    if(self.validacao(entrada, pilha)):
      ...
      print('\n        Valor da pilha atual: ', self.PValor)
      print('        Valor da pilha depois:', self.novoPValor)
...
class Automato():
  ...
  def __init__(self, entrada, pilha, estado):
    ...
    print('\n     Cadeia de entrada: ', entrada)
    self.pilha = Pilha(pilha)
    print('     Valor inicial da pilha: ', pilha)
    self.estado = estado
    pass
  ...
  def print(self):
    print('\n     Cadeia de entrada: ', self.entrada.content())
    print('     Valor topo da Pilha: ', self.pilha.content())
...
def lerArquivo (arquivo):
  ...
  #print('\n     Valor inicial da pilha: ',pilhaInit)
  conjEstados = conteudo[4].split()
  print('     Estados do APD: ',conjEstados)
  iniEstado = conteudo[5].split()[0]
  print('     Estado inicial: ',iniEstado)
  finEstados = conteudo[6].split()
  print('     Estados Finais: ',finEstados)
  transicoes = conteudo[7:]
  if(transicoes[-1] == ''):
    transicoes.pop()
  print('     Iterações existentes: ',transicoes,'\n')
  ii = 0
  while ii < len(transicoes):
    print("               Iteração %d: %s"%(ii, transicoes[ii]) )
    ii = ii + 1
  arq.close()
...
def main(arquivo, entrada):
  ...
  print("\n  |*******************************************|")
  print("  |*********** Reconhecedor de APD ***********|")
  print("  |*******************************************|")
  print("  |****** By Jefter Targino e Jan Carlos******|\n")
    ...
    while (i < tam):
      ...
      if(res == 2):
        ...
        if(tam > 1):
          ...
          break
      i += 1
      #print('       Iteração: ' ,transicoes[i])
      #print('       Iteração: ' ,trans)
  
  # Retorna uma saida para o usuario de acordo com o resultado das execuções
  if(res == 2):
    automatos[i].print()
    print('\n          Pilha vazia ou em estado de aceitação')
  
  elif(res == 0):
    automatos[0].print()
    print('\n          Cadeia digitada não foi aceita')
